...
* `cfn-templates` folder:
    - `project-s3-fs-ingestion.yaml`: CloudFormation template with the SageMaker project
    - `sm-project-sc-portfolio.yaml`: CloudFormation template with product portfolio
    - `sm-sc-policies.yaml`: a managed permission policy to deploy the SageMaker project
* `project-seed-code/s3-fs-ingestion` folder: contains the project source code including the SageMaker pipeline definition code, build scripts for CodeBuild, and source code for the Lambda function.
* `notebooks` folder: contains SageMaker notebooks to experiment with the project

The following sections describe each part of the project and gives examples of source code.
...
- An S3 bucket to keep CodePipeline artifacts
...
- An EventBridge rule to launch the Lambda function whenever a new object is uploaded to the monitored S3 bucket. The events `PutObject` and `CompleteMultipartUpload` are monitored.

The project template is self-sufficient and contains all resources needed to implement the data transformation and ingestion pipeline. It also deploys an CI/CD pipeline to build the SageMaker pipeline every time there is an update on the source code repository. 

### IAM roles and permissions
To launch and use the SageMaker portfolio of products from AWS Service Catalog you need two IAM roles:
- IAM role to launch a SageMaker product from AWS Service Catalog
- IAM role to use resources created by a SageMaker product, such as a CodePipeline pipeline or an EventBridge rule. These resources assume this IAM role to fulfill their tasks.

When you enable SageMaker Projects for Studio users, two default IAM roles are created behind the scenes: `AmazonSageMakerServiceCatalogProductsLaunchRole` and `AmazonSageMakerServiceCatalogProductsUseRole`. These roles are global for your AWS account and used by the SageMaker Project templates. You can use these roles for your custom SageMaker projects or you can create your own roles with a specific set of IAM permissions suited to your requirements.
...
This solution delivers the seed code which contains the SageMaker pipeline definition. A CI/CD workflow as a CodePipeline pipeline is also created as a part of the project. This CodePipeline pipeline is started each time there is a commit to the source code repository.
...
Any resource that is created by the project, is tagged automatically with two tags: `sagemaker:project-name` and `sagemaker:project-id`, allowing for data and resource lineage:
...
#### Delete project
If you don't need the provisioned project any more, to stop incurring charges, you must delete it to clean up the resources the were created by the project.
...
❗ Please note, that a project can create other resources, such as objects in S3 buckets, ML models, feature groups, or CloudFormation stacks. Refer to a specific project documentation to perform a full clean up.

This solution provides a [clean up notebook](notebooks/99-clean-up.ipynb) to delete all resources created by the project.
